...
fn build(sdk_path: Option<&str>, target: &str) {
    ...
    if target == "aarch64-apple-darwin" {
        // Clang uses "arm64" while Rust target triples use "aarch64".
        builder = builder.clang_arg("-target").clang_arg("arm64-apple-darwin");
    } else if target == "aarch64-apple-ios" {
        builder = builder.clang_arg("-target").clang_arg("arm64-apple-ios");
    } else if target == "aarch64-apple-ios-sim" {
        builder = builder
            .clang_arg("-target")
            .clang_arg("arm64-apple-ios-simulator");
    }
    if target.contains("apple-ios") {
